...
from download.download import get_page, main, skip_package
...
def test_get_page_ok():
    ...
    with patch("download.download.requests.get", return_value=resp):
...
def test_no_packages_when_index_page_empty(capsys, download_options):
    resp = response()
    with patch("download.download.parse_args", return_value=download_options):
        with patch("download.download.requests.get", return_value=resp):
            main()
...
def test_count_single_missing_package(capsys, download_options):
    ...
    with patch("download.download.parse_args", return_value=download_options):
        with patch("download.download.requests.get", side_effect=[main_resp, page_resp]):
            main()
            captured = capsys.readouterr()
            assert captured.out == "Package,Releases\nalpha,NA\n"
...
def test_count_single_package_multiple_releases(capsys, download_options):
    ...
    with (
        patch("download.download.parse_args", return_value=download_options),
        patch("download.download.requests.get", side_effect=[main_resp, page_resp]),
    ):
        main()
        captured = capsys.readouterr()
