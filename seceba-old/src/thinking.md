...
Research starting in the 1980s showed that most neurotypical adults undergo a series of fairly
predictable [% g cognitive_transition "cognitive transitions" %] on their way from being a novice to being an expert in any
domain [% b Benner2000 %]. (We say "most" and "neurotypical" because as
with anything involving people, there will be outliers.)

For our purposes, we will say that people are [% g novice "novices" %], [% g competent_practitioner "competent practitioners" %], or [% g expert "experts" %]. Some characteristics that most novices share include
...
What ties these things together is that novices don't have a [% g mental_model "mental model" %] of the problem. A mental
...
*less* good as teachers because of [% g expert_blind_spot "expert blind spot" %]: they know their subject so well that they've
...
You may have heard of "learning styles", i.e., that some people learn better visually,
...
[% b DeBruyckere2015 %]. Like Myers-Briggs personality types,
...
As [% b Kirschner2006 %] and many others have shown, discovery-based learning in which people are
...
should be mentored instead of lectured: they should
...
introspect on their own practice
...
Each of these wrong answers has [% g diagnostic_power "diagnostic power" %]. Like a good unit test, this
...
diagnose misunderstandings so that they can be corrected is called [% g formative_assessment "formative assessment" %]
...
the cognitive architecture of the human brain. On the left is [% g long_term_memory "long-term memory" %] (LTM), which
...
Instead, evolution has given you a second subsystem called [% g short_term_memory "short-term memory" %] (STM) or
[% g working_memory "working memory" %]. (More
...
very small.  [% b Miller1956 %] famously estimated that it could
...
This realization and others have produced the theory of [% g cognitive_load "cognitive load" %], which (among other things)
divides the things you have to do while learning into three categories. The
[% g intrinsic_load "intrinsic load" %] is the thinking that is required by the learning task itself. The
[% g germane_load "germane" %]
(or relevant) load is the other thinking that the problem requires, but which
isn't the focus of the lesson, while the [% g extraneous_load "extraneous load" %] is everything you're
...
Cognitive load theory explains why tools like [Scratch][scratch] work so well: they reduce germane load by
...
In order to handle larger sets of information, our minds create [% g chunking "chunks" %] that only
...
All of this leads to six strategies that
...
One powerful finding in learning research is the [% g hypercorrection_effect "hypercorrection effect" %] [% b Metcalfe2016 %].  Most people don't like to be told
...
One of the strongest predictors of whether or not a neurotypical adult will learn something or not is
whether they are intrinsically motivated to learn it, i.e., whether
they're learning it to satisfy their own goals.  If someone is extrinsically
motivated—for example, if they're taking a course because it's a degree
...
:   Students will learn more if they think the material is useful, but it has to
    ...
    personas like the ones in [% x intro %] is to remind
...
What about demotivation? Again, it turns out that
...
:   If what you do seems to have no effect on the outcome, you soon learn to
    ...
    helplessness, and anyone who has ever wrestled with an intermittent
