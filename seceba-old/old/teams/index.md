...
[% x governance %] for running meetings, making decisions, and resolving conflicts
...
help groups coordinate more effectively, but the groups
...
much you know about your teammates' work.
...
All right, you've formed a team: now what?
How do you decide [% i "teams!allocating work" "allocating work!in teams" %]who does what[% /i %]? How do
...
Many take a third path and decide to leave programming—after all, why play a game that's unfair?
...
2.  It leads to [% i "big bang integration" %][% g big_bang %]big bang integration[% /g %][% /i %],
    in which all the components meet each other for the
...
[% x eval-individual %] are a useful starting point.
...
:   For many years my favorite phrase was, "Why don't we?" Why don't we write a
    ...
    called [% i "feature creep (danger of)" %][% g feature_creep %]feature creep[% /g %][% /i %],
    and has ruined many projects that might otherwise have
...
:   The more you follow a routine, the more your brain will be able to focus on
    ...
    experts. Making these a habit reduce [% i "cognitive
