...
Note: we use the term "topics" in some places because "index" means "the
overall site index page" to Ivy.
"""
...
def make_index(pargs, kwargs, node):
    ...
    # No entries.
    if not (content := util.get_config("topics")):
        return ""
...
def collect():
    ...
    topics = util.make_config("topics")
    ivy.nodes.root().walk(
        lambda node: parser.parse(node.text, {"node": node, "topics": topics})
    )
