...
def figure_def(pargs, kwargs, node):
    ...
    slug = kwargs["slug"]
    img = kwargs["img"]
    alt = util.markdownify(kwargs["alt"], strip_p=True)
    caption = util.markdownify(kwargs["caption"], strip_p=True)
...
def syllabus(pargs, kwargs, node):
    ...
    for slug in ark.site.config["chapters"]:
        ...
        for item in meta[slug]["syllabus"]:
            lines.append(f"- {util.markdownify(item, strip_p=True)}")
    return "\n".join(lines)
...
def toc(pargs, kwargs, node):
    ...
    chapters = _format(ark.site.config["chapters"])
    appendices = _format(ark.site.config["appendices"])
